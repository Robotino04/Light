use std::{time::Instant, io::{stdout, Write}, sync::{Mutex, Arc, atomic::AtomicUsize}, thread};
use light::{image::Image, trace_ray, importing::load_from_blender, image_filters::{average_samples, gamma_correct}};
use rayon::prelude::*;
...
fn main() {
    ...
    ctrlc::set_handler(move || {
        ctrl_c_image.lock().unwrap()
            .apply_filter(|x| average_samples(ctrl_c_next_sample.load(std::sync::atomic::Ordering::Relaxed), x))
